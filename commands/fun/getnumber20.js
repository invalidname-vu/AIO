...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (mode === 'vsbot' && opponent.id !== interaction.client.user.id) {
            return interaction.reply({ content: '🚫 You cannot mention an opponent when playing against me.', ephemeral: true });
        }

        if (mode === 'vsuser' && opponent.id === interaction.client.user.id) {
            return interaction.reply({ content: '🚫 You cannot play against me in VS User mode. Please choose a different opponent.', ephemeral: true });
        }
        ...
        const takeBotTurn = async () => {
            ...
            await interaction.channel.send({ content: `I choose ${botChoice}` });
            ...
            if (activeGames[serverId].score >= targetScore) {
                ...
                const winEmbed = new EmbedBuilder()
                    .setTitle('🏆 I Win!')
                    .setDescription(`Yay! 🎉 I reached ${targetScore} first!\n\nFinal score: ${activeGames[serverId].score}`)
                    .setColor('#FFD700')
                    .setTimestamp();
